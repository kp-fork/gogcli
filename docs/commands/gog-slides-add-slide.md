...
```bash
gog slides (slide) add-slide <presentationId> <image> [flags]
```
...
| `--before` | `string` |  | Insert before this slide ID (appends to end if omitted) |
...
| `--notes-file` | `string` |  | Path to file containing speaker notes |
