...
```bash
gog classroom (class) coursework (work) list (ls) <courseId> [flags]
```
...
| `--order-by` | `string` |  | Order by (e.g., updateTime desc, dueDate desc) |
...
| `--state` | `string` |  | Coursework states filter (comma-separated: DRAFT,PUBLISHED,DELETED) |
