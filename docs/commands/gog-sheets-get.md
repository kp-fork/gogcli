...
```bash
gog sheets (sheet) get (read,show) <spreadsheetId> <range> [flags]
```
...
| `--dimension` | `string` |  | Major dimension: ROWS or COLUMNS |
...
| `--render` | `string` |  | Value render option: FORMATTED_VALUE, UNFORMATTED_VALUE, or FORMULA |
