# `gog slides create-from-template`
...
```bash
gog slides (slide) create-from-template <templateId> <title> [flags]
```
...
| `--exact` | `bool` |  | Use exact string matching instead of {{key}} placeholders |
