# `gog drive changes watch`

> Generated from `gog schema --json`. Do not edit this page by hand; run `make docs-commands`.

Watch Drive changes with a webhook channel

## Usage

```bash
gog drive (drv) changes watch --token=STRING --webhook-url=STRING [flags]
```
...
| `--expiration-ms` | `int64` |  | Unix epoch milliseconds when the channel should expire |
...
| `--token` | `string` |  | Start page token or next page token to watch from |
