...
```bash
gog sheets (sheet) append (add) <spreadsheetId> <range> [<values> ...] [flags]
```
...
| `--copy-validation-from` | `string` |  | Copy data validation from an A1 range or named range (e.g. 'Sheet1!A2:D2' or MyNamedRange) to the appended cells |
