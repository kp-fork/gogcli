...
Download an app-created media item

## Usage

```bash
gog photos (photo) download (dl) <mediaItemId> [flags]
```
...
| `--video` | `bool` |  | Download video bytes with =dv (default auto when metadata says video) |
