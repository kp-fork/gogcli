...
```bash
gog gmail (mail,email) settings autoforward update (edit,set) [flags]
```

## Parent

- [gog gmail settings autoforward](gog-gmail-settings-autoforward.md)
...
| `--email` | `string` |  | Email address to forward to (must be verified first) |
| `--enable` | `bool` |  | Enable auto-forwarding |
