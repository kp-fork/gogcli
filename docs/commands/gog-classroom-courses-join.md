...
```bash
gog classroom (class) courses (course) join (enroll) <courseId> [flags]
```
...
| `--enrollment-code` | `string` |  | Enrollment code (student joins only) |
...
| `--role` | `string` | student | Role to join as: student\|teacher |
| `--select`<br>`--pick`<br>`--project` | `string` |  | In JSON mode, select comma-separated fields (best-effort; supports dot paths). Desire path: use --fields for most commands. |
| `--user` | `string` | me | User ID or email to join |
