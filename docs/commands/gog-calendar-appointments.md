...
List appointment schedule events

## Usage

```bash
gog calendar (cal) appointments (appointment-schedules,appt) [<calendarId> ...] [flags]
```
...
| `--all` | `bool` |  | Fetch appointment schedules from all calendars |
...
| `--from` | `string` |  | Start time (RFC3339 with timezone, date, or relative) |
