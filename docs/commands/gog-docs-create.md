...
```bash
gog docs (doc) create (add,new) <title> [flags]
```
...
| `--file` | `string` |  | Markdown file to import. Supports inline images from public HTTPS URLs via ![alt](url); append {width=N height=N} to control size in points. |
