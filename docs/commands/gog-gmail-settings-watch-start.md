...
Start Gmail watch for Pub/Sub

## Usage

```bash
gog gmail (mail,email) settings watch start (begin) [flags]
```
...
| `--label` | `[]string` |  | Label IDs or names (repeatable, comma-separated) |
...
| `--topic` | `string` |  | Pub/Sub topic (projects/.../topics/...) |
| `--ttl` | `string` |  | Renew after duration (seconds or Go duration) |
