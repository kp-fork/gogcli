...
```bash
gog gmail (mail,email) thread (threads,read) modify (update,edit,set) <threadId> [flags]
```
...
| `--add` | `string` |  | Labels to add (comma-separated, name or ID) |
...
| `--remove` | `string` |  | Labels to remove (comma-separated, name or ID) |
