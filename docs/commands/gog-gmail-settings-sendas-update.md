# `gog gmail settings sendas update`
...
```bash
gog gmail (mail,email) settings sendas update (edit,set) <email> [flags]
```

## Parent

- [gog gmail settings sendas](gog-gmail-settings-sendas.md)
...
| `--display-name` | `string` |  | Name that appears in the From field |
...
| `--make-default` | `bool` |  | Make this the default send-as address |
