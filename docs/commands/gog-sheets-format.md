...
```bash
gog sheets (sheet) format <spreadsheetId> <range> [flags]
```
...
| `--format-fields` | `string` |  | Format field mask (eg. userEnteredFormat.textFormat.bold or textFormat.bold) |
| `--format-json` | `string` |  | Cell format as JSON (Sheets API CellFormat) |
