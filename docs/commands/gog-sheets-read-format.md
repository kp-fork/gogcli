...
```bash
gog sheets (sheet) read-format (get-format,format-read) <spreadsheetId> <range> [flags]
```
...
| `--effective` | `bool` |  | Read effective format instead of user-entered format |
