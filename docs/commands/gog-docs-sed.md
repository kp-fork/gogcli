...
Regex find/replace (sed-style: s/pattern/replacement/g)

## Usage

```bash
gog docs (doc) sed <docId> [<expression>] [flags]
```
...
| `-e`<br>`--expressions` | `[]string` |  | Additional sed expressions (repeatable) |
| `-f`<br>`--file` | `string` |  | Read sed expressions from file (one per line, # comments) |
